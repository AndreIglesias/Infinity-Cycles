...
class Juego():
    ...
    def __init__(self, controller, multPlayer = False, score1 = 0, player = 1):
        ...
        self.w = Canvas(self.master, width=1100, height=650, bg="gray10", highlightbackground="black")
        ...
        def generarPowerUps(cantidad):
            powerUps = []
            tipos = (("#fff9af", "#ff9900", "timer"),("#800080", "#ff9900", "+Retrocesos"),("#fede12", "#ff9900", "Puntos_x2"),("#ff6666", "#ff9900", "+Rango"),("#8c0000", "#ff9900", "-Obstaculo")) #(Relleno, borde, tag)
            for i in range(0,cantidad):
            ...
            powerUps.append(self.w.create_oval(x2, y2, x2+30, y2+30, fill=tipos[MegapowerUp][0], outline=tipos[MegapowerUp][1], width=3, tags=tipos[MegapowerUp][2]))
        ...
        self.timer = self.hud.create_text((408, 22), text="Texto", font=("", "12", "bold"), fill="#0048b5")
        self.hud.create_line(460, 0 , 460, 50 , width=3)

        self.textoPU = self.hud.create_text(530, 22, text="???", font=("", "12", "bold"), fill="black")
        ...
        if(self.settings["sounds"]["value"] == 1):
            try: winsound.PlaySound("assets/sound.wav", winsound.SND_ASYNC)
    ...
    def detener(self):
        ...
        if(self.multPlayer):
            self.master.destroy()
            self.controller.endgameM(self.puntaje, self.score1, self.player)
        else:
        ...
        if(c >= self.minimo and c <= self.maximo):
            ...
            for i in colision:
                if self.w.gettags(i) != ():
                    ...
                    if self.w.gettags(i)[0] == 'target' or self.w.gettags(i)[0] == '+Retrocesos' or self.w.gettags(i)[0] == 'timer' or self.w.gettags(i)[0] == 'Puntos_x2' or self.w.gettags(i)[0] == '+Rango' or self.w.gettags(i)[0] == '-Obstaculo':
            ...
            if lista.count('target') >= 1:
                ...
                for i in self.objetivos:
                    if i[0] == colision[index]:
                        if(self.doblePuntaje==1):
                            self.puntajeMax += i[1]*2-i[1]
                            self.puntaje += i[1]*2
                            self.doblePuntaje=0
                            self.hud.itemconfig(self.textoPU,text="Puntos x2", fill="red")
                        else:
                            self.puntaje += i[1]
            ...
            if lista.count("Puntos_x2") >=1:
                self.doblePuntaje=1
                self.hud.itemconfig(self.textoPU,text="Puntos x2", fill="green")
                for i in self.powerUps:
                    if i == colision[index]:
                        self.w.delete(i)

            if lista.count("+Rango") >=1:
                self.maximo = 65
                self.hud.itemconfig(self.textoPU,text="Rango +", fill="green")
                for i in self.powerUps:
                    if i == colision[index]:
                        self.w.delete(i)


            if lista.count("-Obstaculo") >=1:
                for i in self.powerUps:
                    if i == colision[index]:
                        self.w.delete(i)
                        if(self.dif<3):
                            self.w.delete(self.obstaculos[0],self.obstaculos[1])
                        elif(self.dif==3):
                            self.w.delete(self.obstaculos[0],self.obstaculos[1],self.obstaculos[2])
                        else:
                            self.w.delete(self.obstaculos[0],self.obstaculos[1],self.obstaculos[2],self.obstaculos[3])
                        self.hud.itemconfig(self.textoPU,text="Obstáculos -", fill="red")

            if lista.count("+Retrocesos") >=1:
                self.retrocesosMax +=2
                self.hud.itemconfig(self.textoPU,text="Retrocesos +", fill="green")
                for i in self.powerUps:
                    if i == colision[index]:
                        self.w.delete(i)
